...
and multiply numerical data, applying probability allows for some
...
The expression \lq{}random variable\rq{} is used very loosely and in a
way that can be confusing; however, it does have an exact meaning. A
random variable $X$ is a map from a sample space to a set of numerical
values. For now we will deal with a discrete random variables, where
those values are discrete. As an example consider the sample space for
flipping a coin three times; the sample space will have eight
elements, \{HHH, HHT, HTH, THH, HTT, THT, TTH, TTT\}. One random
variable, say $X$, on this space would be the number of heads. This
random variable would map HHH to three and TTH to one.
...
all the outcomes with value $x$. It is the probability of the event:
\begin{equation}
\{a\in \mathcal{X}|X(a)=x\}
\end{equation}
...
\end{tabular}
\end{center}
We won't discuss it much but you could also work out a \textsl{joint distribution}, this is the
...
define the expectation value. If $g(x)$ is a function we define the \textsl{expected value} of $g(X)$ as
\begin{equation}
...
then the expected value is equal to the mean. 

It is also referred to as the \textsl{first moment}; the first bit is
because it is the expectation value for the first power of $X$.
...
\end{equation}
If $p(x)$ represents the frequencies, the this is the square of the
...
properties of the underlying arithmetic. First, scalar multiplication, for a constant $c$:
\begin{equation}
...
\end{equation}
Finally, it is additive: for $g_1$ and $g_2$ two different functions
\begin{eqnarray}
...
Incidentally $\langle X^2\rangle$ is called the \textsl{second
  moment}, the variance is also called the \textsl{central second
  moment}; the \lq{}central\rq{} indicates that it is the second
moment you get if you take away the mean first.
